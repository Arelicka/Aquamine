...
class Level extends VectorHashUtils implements ChunkManager, Metadatable{
	...
	public function setBlock(Vector3 $pos, Block $block, bool $direct = false, bool $update = true) : bool{
		...
		if($this->getChunk($pos->getFloorX() >> 4, $pos->getFloorZ() >> 4, true)->setBlock($pos->x & 0x0f, $pos->y, $pos->z & 0x0f, $block->getId(), $block->getDamage())){
			...
			if($direct === true){
				$this->sendBlocks($this->getChunkPlayers($pos->getFloorX() >> 4, $pos->getFloorZ() >> 4), [$block], UpdateBlockPacket::FLAG_ALL_PRIORITY);
			...
			foreach($this->getChunkListeners($pos->getFloorX() >> 4, $pos->getFloorZ() >> 4) as $listener){
	...
	public function onTileChanged(Tile $tile) : void{
		foreach($this->getChunkListeners($tile->getFloorX() >> 4, $tile->getFloorZ() >> 4) as $listener){
	...
	public function addTile(Tile $tile){
		...
		if(($chunk = $this->getChunk($tile->getFloorX() >> 4, $tile->getFloorZ() >> 4)) !== null){
			$chunk->addTile($tile);
		}
	}
	...
	public function getSafeSpawn($spawn = null){
		...
		if($spawn instanceof Vector3){
			...
			$v = $spawn->floor();
			$chunk = $this->getChunk($v->getFloorX() >> 4, $v->getFloorZ() >> 4, false);
			$x = $v->x & 0x0f;
			$z = $v->z & 0x0f;
			if($chunk !== null){
				$y = (int) min($max - 2, $v->y);
